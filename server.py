    ...
    while True:
        try:
            ...
            if message == 'LOGOUT':
                ...
                break
            if message != '':
                final_msg = f"{username}~{message}"  # Format the message
                add_message_to_chat_activity(final_msg)  # Add to chat activity
...
def remove_client(client, username):
    ...
    update_connected_clients()  # Update the connected clients list
    ...
    while True:
        try:
            ...
            if username != '':
                ...
                add_message_to_chat_activity(prompt_message)  # Add to chat activity
                add_message_to_server_log(prompt_message)
                send_message_to_all(prompt_message)
                update_connected_clients()  # Update the connected clients list
                break
...
def stop_server():
    ...
    if is_server_running:
        ...
        try:
            ...
            update_connected_clients()  # Clear the connected clients list when the server stops
        except:
...
def add_message_to_server_log(message):
    ...
    server_log_box.see(tk.END)  # Auto-scroll to the end
